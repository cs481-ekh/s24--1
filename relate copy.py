#####
#NOTES: PETER BUSCHE
    # 1) changed alot to import statements
    # 2) once we got import statements to work, changed all instances of Numeric in code
        # fixed any other isses related to Numeric and MLab
...
# from numpy.linalg import eigenvectors
from numpy.linalg import eig            #PeterBusche
...
class dataMatrix:
    ...
    def idMatrix(self, data):
        iMatrix={}
        for i in list(data.keys()): iMatrix[i]={}
        return iMatrix
...
class rMatrix(dataMatrix):
    ...
    def findFounders(self, count):
        ...
        for i in list(self.dict.keys()):
            ...
            if self.dict[i][0] == missing and self.dict[i][1] == missing: founders += [i]
                ...
                if self.dict[i][0] not in list(self.dict.keys()) and self.dict[i][0] not in founders and self.dict[i][0] != missing:
                    founders += [ self.dict[i][0] ]
                    # Add father to list of egos
                    self.dict[self.dict[i][0]] = [missing, missing, self.fieldDef['male'], missing]
                if self.dict[i][1] not in list(self.dict.keys()) and self.dict[i][1] not in founders and self.dict[i][1] != missing:
    ...
    def FgALL_OLD(self):
        ...
        theKeys=list(self.rMatrix.keys())
        theKeys.sort()
        for i in theKeys:
            sumALL=0
            sizeALL=0
            ...
            if self.fieldDef['allAlive'] or self.dict[i][3] == self.fieldDef['alive']: # this filters out dead people
                for j in list(self.rMatrix[i].keys()):
            ...
            if father in self.rMatrix and mother in self.rMatrix:
    ...
    def idMatrix2Array(self, d):
        # Helper function to convert the dictionary 'array' of relatedness values to a NumPy array
        self.IDS = list(d.keys()); self.IDS.sort()
        n = len(self.IDS)
        array = np.identity(n, dtype=float)
        for i in range(n):
            for j in range(n):
    ...
    def PCA2(self):
        ...
        self.M = self.a  # self.idMatrix2Array(self.rMatrix)  # Convert dictionary 'array' of relatedness values to a NumPy array
        # Center the data, and negate for dissimilarities
        self.M = np.mean(self.M) - self.M
        # Compute eigenvectors of covariance matrix of M
        self.evals, self.evecs = np.linalg.eig(np.cov(self.M))
        self.evals = self.evals.astype(float)  # make sure all eigenvalues are real (symmetric matrix, so should be)
        # sort the eigenvalues and eigenvectors, descending order
        order = np.argsort(self.evals)[::-1]
        self.evecs = self.evecs[:, order]
    ...
    def PCAPlot2(self, dim):
        featureVector = np.transpose(self.evecs[:, :dim]).astype(float)  # Take first 'dim' eigenvectors and transpose
        return np.transpose(np.dot(featureVector, np.transpose(self.M)))
        # ===========================================OLD=================================================
        # def PCAPlot2(self, dim):
        #     featureVector = Numeric.transpose(Numeric.take(self.evecs, list(range(dim)), 1)).astype(Numeric.Float) # Take first 'dim' eigenvectors and transpose
    ...
    def PCAPlot(self, dim):
        #print 'v: ', self.v[:,:dim]
        #print 'u: ', self.u[:,:dim]
        return np.dot(np.transpose(self.M), self.u[:, :dim])
...
class Groups(dataMatrix):
    ...
    def averageGroupRelatedness(self):
        ...
        groupIDs = list(self.groupDict.keys())
        groupAvg = {}
        for i in groupIDs:
            rSum, count = 0, 0
...
class KinGroups(dataMatrix):
    ...
    def kinCount(self, count, functionList, allAlive = False):
        idList = list(self.dict.keys())
        idList.sort()
        self.errors=repr(functionList)
        matrix=[['ego']]
        ...
        if 1: # Leave if_statement here in case we want to make this an option
            for j in kinGroups:
                for i in j:
                    for k in i[:]: # Makes temp copy of i so we can delete stuff from the real i
                        # if k not an ego
                        if k not in self.dict: i.remove(k)
...
class Lineages(dataMatrix):
    ...
    def computeLineages(self):
        ...
        keys = list(egoPatrilineage.keys())
        keys.sort()
        for i in keys:
    ...
    def lineageStats(self, lineages, lineageType):
        ...
        stats['No. not belonging to any ' + lineageType] = repr(lineages[self.missing][0])
        #for i in lineages:
        ...
        stats['No. of '+ lineageType +'s'] = repr(len(lin))
        stats['Smallest '+ lineageType] = repr(min(lin))
        stats['Largest '+ lineageType] = repr(max(lin))
        sum = 0
        for i in lin: sum += i
